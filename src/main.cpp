...
#if defined(NDEBUG)
#error "Exor cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uexor) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zEXOR spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zEXOR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zEXOR spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zEXOR without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zEXOR spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zEXOR spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zexor");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zEXOR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 274999.8992 * COIN;	// 200 block pre-mine = 54999979.84 EXOR
	else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 20360)
		nSubsidy = 0.001 * COIN;	// 14-day "fair launch" swap period = 20.16 EXOR
	else if (nHeight > 20360 && nHeight <= 545960)
		nSubsidy = 5.4 * COIN;	// Year 1 rewards = 2838240 EXOR
	else if (nHeight > 545960 && nHeight <= 1071560)
		nSubsidy = 4.59 * COIN;	// Year 2 rewards = 2412504 EXOR
	else if (nHeight > 1071560 && nHeight <= 1597160)
		nSubsidy = 3.9015 * COIN;	// Year 3 rewards = 2050628.40 EXOR
	else if (nHeight > 1597160 && nHeight <= 2122760)
		nSubsidy = 3.3163 * COIN;	// Year 4 rewards = 1743047.28 EXOR
	else if (nHeight > 2122760 && nHeight <= 2648360)
		nSubsidy = 2.8188 * COIN;	// Year 5 rewards = 1481561.28 EXOR
	else if (nHeight > 2648360 && nHeight <= 3173960)
		nSubsidy = 2.3960 * COIN;	// Year 6 rewards = 1259337.6 EXOR
	else if (nHeight > 3173960 && nHeight <= 3699560)
		nSubsidy = 2.0366 * COIN;	// Year 7 rewards = 1070436.96 EXOR
	else if (nHeight > 3699560 && nHeight <= 4225160)
		nSubsidy = 1.7311 * COIN;	// Year 8 rewards = 909866.16 EXOR
	else if (nHeight > 4225160 && nHeight <= 4750760)
		nSubsidy = 1.4714 * COIN;	// Year 9 rewards = 773367.84 EXOR
	else if (nHeight > 4750760 && nHeight <= 5276360)
		nSubsidy = 1.2507 * COIN;	// Year 10 rewards = 657367.92 EXOR
    else
        nSubsidy = 0;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZEXORStake)
{
    int64_t ret = 0;
	...
	if (nHeight <= (Params().LAST_POW_BLOCK() + (7*1440)))
        ret = 0;	// Do not start masternode payments until after 1 week into the "fair launch"
    else
        ret = blockValue * .80;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Exor
...
{
    RenameThread("exor-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZEXORMinted()
{
...
void RecalculateZEXORSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zEXOR supply
...
bool RecalculateEXORSupply(int nHeightStart)
{
...
bool UpdateZEXORSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZEXORMinted();
        RecalculateZEXORSpent();
        RecalculateEXORSupply(Params().Zerocoin_StartHeight());
    }

    //Track zEXOR money supply in the block index
    if (!UpdateZEXORSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zEXOR supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zExorSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    //The reward increase after the "fair launch" needs to be handled differently as only reward decreases are typically allowed
    if (pindex->nHeight == (20360+1))
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert EXOR to zEXOR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zEXOR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zEXOR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zEXOR stake block is less than allowed start height", __func__);

    if (CZExorStake* zEXOR = dynamic_cast<CZExorStake*>(stake)) {
        CBlockIndex* pindexFrom = zEXOR->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zEXOR stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zEXOR stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zEXOR->GetValue()));
        if (nChecksum200 != zEXOR->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zEXOR->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZEXOR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zEXOR fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> exorInputs;
        std::vector<CTxIn> zEXORInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zEXORInputs.push_back(stakeIn);
            }else{
                exorInputs.push_back(stakeIn);
            }
        }
        const bool hasEXORInputs = !exorInputs.empty();
        const bool hasZEXORInputs = !zEXORInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasEXORInputs)
                    ...
                    for (const CTxIn& exorIn : exorInputs){
                        if(exorIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : exorInputs) {
            ...
            // Now that this loop if completed. Check if we have zEXOR inputs.
            if(hasZEXORInputs){
                for (const CTxIn& zExorInput : zEXORInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zExorInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zexor");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zEXOR mints and %d zEXOR spends\n", __func__, nMints, nSpends);
