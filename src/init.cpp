...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "exor.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "exord.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the EXOR and zEXOR money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 51572, 51574));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EXOR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EXOR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, exor, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EXOR/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Exor specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-exorstake=<n>", strprintf(_("Enable or disable staking functionality for EXOR inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zexorstake=<n>", strprintf(_("Enable or disable staking functionality for zEXOR inputs (0-1, default: %u)"), 1));
    ...
    if (Params().ZeroCoinEnabled()) {
		...
		strUsage += HelpMessageOpt("-backupzexor=<n>", strprintf(_("Enable automatic wallet backups triggered after each zEXOR minting (0-1, default: %u)"), 1));
		strUsage += HelpMessageOpt("-zexorbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zEXOR backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeexoramount=<n>     " + strprintf(_("Keep N EXOR anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
		   FormatParagraph(_("Copyright (C) 2019 The Exor Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Exor is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Exor Core is shutting down."));
...
#endif
    // Make sure only a single Exor process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Exor Core is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Exor Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Exor Core to complete") << "\n";
        ...
        zwalletMain = new CzEXORWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zEXORWallet
        uiInterface.InitMessage(_("Syncing zEXOR wallet..."));

        bool fEnableZExorBackups = GetBoolArg("-backupzexor", true);
        pwalletMain->setZExorAutoBackups(fEnableZExorBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zexorTracker->Init();
...
//    }
//
//    nAnonymizeExorAmount = GetArg("-anonymizeexoramount", 0);
//    if (nAnonymizeExorAmount > 999999) nAnonymizeExorAmount = 999999;
//    if (nAnonymizeExorAmount < 2) nAnonymizeExorAmount = 2;
    ...
    LogPrintf("Anonymize Exor Amount %d\n", nAnonymizeExorAmount);
    ...
    /* Denominations
       ...
       For example:
       1EXOR+1000 == (.1EXOR+100)*10
       10EXOR+10000 == (1EXOR+1000)*10
    */
