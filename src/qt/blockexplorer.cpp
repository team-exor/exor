...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::EXOR, nValue);
...
static std::string makeHTMLTableRow(const std::string* pCells, int n, int nRowNum)
{
    std::string Result = "<tr>";
    for (int i = 0; i < n; i++) {
        Result += "<td class=\"d" + std::string((n>2) ? std::string((nRowNum>0) ? "z" : "t") : "") + utostr(i) + "\">";
        Result += pCells[i];
        Result += "</td>";
    }
    Result += "</tr>";
...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColumns)
{
    ...
    for (int i = 0; i < nRows; i++)
        Table += makeHTMLTableRow(pCells + i * nColumns, nColumns, i);
    Table += "</table>";
    return Table;
}

static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(), const std::string& Prepend = std::string(), int64_t* pSum = NULL, int nRowNum = -1)
{
    ...
    if (!Highlight.empty()) {
        ...
        *pSum += Delta;
        ...
        return makeHTMLTableRow(List, n, nRowNum);
    }
    return makeHTMLTableRow(List + 1, n - 1, nRowNum);
}
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    std::string TxContent = table + makeHTMLTableRow(TxLabels, sizeof(TxLabels) / sizeof(std::string), 0);
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        const CTransaction& tx = block.vtx[i];
        TxContent += TxToRow(tx, CScript(), std::string(), NULL, (i+1));
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    std::string InputsContent = makeHTMLTableRow(InputsContentCells, sizeof(InputsContentCells) / sizeof(std::string), 0);
    std::string OutputsContentCells[] = {_("#"), _("Redeemed in"), _("Address"), _("Amount")};
    std::string OutputsContent = makeHTMLTableRow(OutputsContentCells, sizeof(OutputsContentCells) / sizeof(std::string), 0);
    ...
    if (tx.IsCoinBase()) {
        ...
        InputsContent += makeHTMLTableRow(InputsContentCells, sizeof(InputsContentCells) / sizeof(std::string), 1);
    } else
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            InputsContent += makeHTMLTableRow(InputsContentCells, sizeof(InputsContentCells) / sizeof(std::string), (i+1));
        }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        OutputsContent += makeHTMLTableRow(OutputsContentCells, sizeof(OutputsContentCells) / sizeof(std::string), (i+1));
    }
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (exor.conf).");
            QMessageBox::warning(this, "Exor Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#ffffff;}\n a, span { font-family: monospace; }\n span.addr {color:#ff0000; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid #000000;}\n td.d0, td.dt0, td.dt1, td.dt2, td.dt3, td.dt4, td.dt5, td.dt6, td.dt7 {font-weight: bold; color:#f2c74e;}\n h2, h3 { white-space:nowrap; color:#ffffff;}\n a { color:#ffa500; }\n a.nav {color:#ffffff; text-decoration:none;}\n";
