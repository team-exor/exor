                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEXOR ought to be enough for anybody." - Bill Gates, 2017
    ui->zEXORpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEXORSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEXORpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEXOR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
	...
	if(!Params().ZeroCoinEnabled() || GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXOR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zEXOR, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEXOR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEXOR in ") +
...
void PrivacyDialog::on_pushButtonSpendzEXOR_clicked()
{
	...
	if(!Params().ZeroCoinEnabled() || GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXOR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zEXOR, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zEXOR
        sendzEXOR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEXOR
    sendzEXOR();
}

void PrivacyDialog::on_pushButtonZExorControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZExorControlDialog* zExorControl = new ZExorControlDialog(this);
    zExorControl->setModel(walletModel);
    zExorControl->exec();
}

void PrivacyDialog::setZExorControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzExorSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEXOR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Exor Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEXORpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEXOR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EXOR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEXOR</b>";
    ...
    // use mints from zEXOR selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZExorControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZExorControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zEXOR require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zEXOR"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZEXOR_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zEXOR require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zEXOR"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zEXOR was spent successfully update the addressbook with the label
    ...
    // Clear zexor selector in case it was used
    ZExorControlDialog::setSelectedMints.clear();
    ui->labelzExorSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEXOR Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Exor, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEXOR Mint");
                               ...
                               const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zexorTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEXOR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEXOR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEXOR "));
    ui->labelzEXORAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zEXOR </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zEXOR </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzEXORSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzEXOR->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zEXOR
        ui->pushButtonMintzEXOR->setEnabled(false);
        ui->pushButtonMintzEXOR->setToolTip(tr("zEXOR is currently disabled due to maintenance."));

        // Spend zEXOR
        ui->pushButtonSpendzEXOR->setEnabled(false);
        ui->pushButtonSpendzEXOR->setToolTip(tr("zEXOR is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zEXOR
        ui->pushButtonMintzEXOR->setEnabled(true);
        ui->pushButtonMintzEXOR->setToolTip(tr("PrivacyDialog", "Enter an amount of EXOR to convert to zEXOR", 0));

        // Spend zEXOR
        ui->pushButtonSpendzEXOR->setEnabled(true);
        ui->pushButtonSpendzEXOR->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
