...
#include "ui_zexorcontroldialog.h"
...
std::set<std::string> ZExorControlDialog::setSelectedMints;
std::set<CMintMeta> ZExorControlDialog::setMints;

ZExorControlDialog::ZExorControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZExorControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZExorControlDialog::~ZExorControlDialog()
{
    delete ui;
}

void ZExorControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZExorControlDialog::updateList()
{
...
void ZExorControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZExorControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZExor_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZExorControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZExorControlDialog::GetSelectedMints()
{
...
void ZExorControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
