...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EXOR);
	...
	fEnableZeromint = Params().ZeroCoinEnabled() && settings.value("fZeromintEnable").toBool();
    ...
    if (!settings.contains("nAnonymizeExorAmount"))
        settings.setValue("nAnonymizeExorAmount", 1000);

    nAnonymizeExorAmount = settings.value("nAnonymizeExorAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeExorAmount"))
        SoftSetArg("-anonymizeexoramount", settings.value("nAnonymizeExorAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in exor.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeExorAmount:
            return QVariant(nAnonymizeExorAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeExorAmount:
            nAnonymizeExorAmount = value.toInt();
            settings.setValue("nAnonymizeExorAmount", nAnonymizeExorAmount);
            emit anonymizeExorAmountChanged(nAnonymizeExorAmount);
            break;
        case CoinControlFeatures:
