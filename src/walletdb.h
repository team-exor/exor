...
public:
    ...
    bool WriteZEXORSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZEXORSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZEXORSeed_deprecated(uint256& seed);
    bool EraseZEXORSeed();
    bool EraseZEXORSeed_deprecated();

    bool WriteZEXORCount(const uint32_t& nCount);
    bool ReadZEXORCount(uint32_t& nCount);
