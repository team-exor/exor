...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT50000IFMN = 3,
    ONLY_NONDENOMINATED_NOT50000IFMN = 4, // ONLY_NONDENOMINATED and not 50000 EXOR at the same time
    ONLY_50000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zEXOR send
enum ZerocoinSpendStatus {
    ZEXOR_SPEND_OKAY = 0,                            // No error
    ZEXOR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEXOR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEXOR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEXOR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEXOR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEXOR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEXOR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEXOR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEXOR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEXOR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEXOR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEXOR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEXOR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEXOR_SPENT_USED_ZEXOR = 14,                      // Coin has already been spend
    ZEXOR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEXOR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZExorBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZEXOROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzexorAuto) const;
    ...
    */
    ...
    CzEXORWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzEXORTracker> zexorTracker;
    ...
    void setZWallet(CzEXORWallet* zwallet)
    {
        zwalletMain = zwallet;
        zexorTracker = std::unique_ptr<CzEXORTracker>(new CzEXORTracker(strWalletFile));
    }

    CzEXORWallet* getZWallet() { return zwalletMain; }
    ...
    void setZExorAutoBackups(bool fEnabled)
    {
