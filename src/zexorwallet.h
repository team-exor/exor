...
#ifndef EXOR_ZEXORWALLET_H
#define EXOR_ZEXORWALLET_H
...
public:
    CzEXORWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZEXOR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZEXOR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //EXOR_ZEXORWALLET_H
