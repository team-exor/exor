...
CzEXORWallet::CzEXORWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zexor seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEXORSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEXORSeed_deprecated()) {
                    LogPrintf("%s: Updated zEXOR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zexor seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zexor wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEXORWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEXORCount(nCountLastUsed);
    else if (!walletdb.ReadZEXORCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEXORWallet::Lock()
{
    seedMaster = 0;
}

void CzEXORWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEXORWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEXOR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEXORWallet::LoadMintPoolFromDB()
{
...
void CzEXORWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEXORWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEXORWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zexorTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEXORWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEXOR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zexorTracker which also adds to database
    pwalletMain->zexorTracker->Add(dMint, true);
...
void CzEXORWallet::SeedToZEXOR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEXORWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEXORWallet::GenerateDeterministicZEXOR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEXORWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEXORWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
