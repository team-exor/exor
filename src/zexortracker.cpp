...
CzEXORTracker::CzEXORTracker(std::string strWalletFile)
{
...
CzEXORTracker::~CzEXORTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEXORTracker::Init()
{
...
bool CzEXORTracker::Archive(CMintMeta& meta)
{
...
bool CzEXORTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEXORTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEXORTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEXORTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzEXORTracker::GetSerialHashes()
{
...
CAmount CzEXORTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEXORTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEXORTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEXORTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEXORTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEXORTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEXORTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEXORTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEXORTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEXORTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEXORTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEXORTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEXORTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEXORTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEXORTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEXORTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEXORTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzexor from DB\n", __func__, listDeterministicDB.size());
    }
