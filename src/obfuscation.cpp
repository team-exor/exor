    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Exor and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 EXOR per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::NewBlock()
{
    ...
    //if we're processing lots of blocks, we'll just leave
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeExorAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100EXOR+1 ( bit on if present )
    // bit 1 - 10EXOR+1
    // bit 2 - 1EXOR+1
    // bit 3 - .1EXOR+1
    // bit 3 - non-denom
